...
const Item = ({ ifExists,id, color, shadow, title, text, img, btn, rating, price }) => {
  return (
    <>
      <div
        className={`relative bg-gradient-to-b ${color} ${shadow} grid items-center ${ifExists ? 'justify-items-start' : 'justify-items-center'} rounded-xl py-4 px-5 transition-all duration-700 ease-out  w-full hover:scale-105`}
      >
        <div className={`grid items-center ${ifExists ? 'justify-items-start' : 'justify-items-center'}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" className="bg-white 90 blur-effect-theme button-theme px-2 py-1 shadow-sky-200 text-sm text-black">
              {btn}
            </button>
          </div>
        </div>
